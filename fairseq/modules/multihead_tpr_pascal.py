...
class MultiheadTPRPascal(nn.Module):
    ...
    def __init__(
        ...
        self.parent_ignoring = parent_ignoring
        self.binomial = torch.distributions.binomial.Binomial(probs=1-parent_ignoring)
    ...
    def reset_parameters(self):
        ...
        if self.num_roles is not None and self.role_weights_input != 'dependency':
    ...
    def normpdf(self, xs, parents, sd):
        var = sd**2
        denom = (2*np.pi*var)**.5
        return torch.exp(-(xs - parents.unsqueeze(-1))**2/(2*var))/denom

    def unipdf(self, xs, parents, w):
        return torch.lt(torch.abs(xs - parents.unsqueeze(-1)), w+1).type(torch.cuda.FloatTensor)
    ...
    def forward(
        ...
        rng = torch.cuda.FloatTensor(range(parents.size(1)))
        pascal_weights = self.weight_fn(rng, parents, self.weight_param).repeat(1, self.num_pascal_heads, 1).view(-1, parents.size(1), parents.size(1))
        if self.training and self.parent_ignoring > 0.:
            samples = self.binomial.sample(pascal_weights.size()[:2]).type(torch.cuda.FloatTensor)
            inv_samples = 1 - samples
            pascal_weights = pascal_weights * samples.unsqueeze(2) + inv_samples.unsqueeze(2)
        attn_weights[-self.num_pascal_heads * bsz:] *= pascal_weights
