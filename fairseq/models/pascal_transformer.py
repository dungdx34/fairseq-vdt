...
class PascalTransformerEncoder(FairseqTagsEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, tags_dictionary, left_pad=True):
        ...
        self.layers.extend(
            [self.build_encoder_layer(args, h) if h != 0 else TransformerEncoderLayer(args) for h in self.encoder_pascal_heads]
        )
    ...
    def build_encoder_layer(self, args, h):
        return PascalTransformerEncoderLayer(args, h)
    ...
    def forward(self, src_tokens, src_lengths, src_tags, return_all_hiddens: bool = False):
        ...
        parents = torch.FloatTensor(np.vectorize(lambda e: self.map_dictionary.get(e, maxlen))(src_tags))
