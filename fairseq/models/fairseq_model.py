...
from fairseq.models import FairseqDecoder, FairseqEncoder, FairseqTagsEncoder
...
class FairseqTagsModel(BaseFairseqModel):
    ...
    def forward(self, src_tokens, src_lengths, src_tags, prev_output_tokens, **kwargs):
        """
        Run the forward pass for an encoder-decoder model.
...
class FairseqDepsTagsModel(BaseFairseqModel):
    """Base class for encoder-decoder models.
    ...
    def __init__(self, encoder, decoder):
        ...
        self.encoder = encoder
        self.decoder = decoder
        assert isinstance(self.encoder, FairseqDepsTagsEncoder)
        assert isinstance(self.decoder, FairseqDecoder)

    def forward(self, src_tokens, src_lengths, src_tags, src_deps, prev_output_tokens, **kwargs):
        """
        Run the forward pass for an encoder-decoder model.
        ...
        Returns:
            tuple:
                ...
                - a dictionary with any model-specific outputs
        """
        encoder_out = self.encoder(src_tokens, src_lengths=src_lengths, src_tags=src_tags, src_deps=src_deps, **kwargs)
        decoder_out = self.decoder(
            prev_output_tokens, encoder_out=encoder_out, **kwargs
        )
        return decoder_out
    ...
    def extract_features(self, src_tokens, src_lengths, src_tags, src_deps, prev_output_tokens, **kwargs):
        """
        Similar to *forward* but only return features.
