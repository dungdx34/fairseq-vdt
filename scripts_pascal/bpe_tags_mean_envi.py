...
parsers = {'en': CoreNLPDependencyParser(url='http://localhost:6666'),
           ...
           'zh': CoreNLPDependencyParser(url='http://localhost:6668')}
...
def annotate(sentences):
    ...
    for ix, sent in enumerate(sentences):
        ...
        new_split = [s for s in split1]
        if len(split1) != len(sent_token_tags):
            if len(split1) < len(sent_token_tags):
                for i in range(len(sent_token_tags)-len(split1)):
                    new_split.append('.')
        ...
        word_tags.append([(html.unescape(new_split[i]), tag) for i, (tok, tag) in enumerate(sent_token_tags)])
...
def tags_to_bpe(sent, word_tags, separator='@@'):
    ...
    for t in sent_tags:
        if t in word2tok:
            new_sent_tags.append(t)

    sent_tags = [word2tok[t] for t in new_sent_tags]
